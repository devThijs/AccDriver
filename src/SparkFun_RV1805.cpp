//RV1805 code edited to work with atmel i2c drivers
...
{
	return I2C_0_read1ByteRegister(RV1805_ADDR, uint8_t addr);
...
{
	I2C_0_write1ByteRegister(RV1805_ADDR, addr, val);
...
bool RV1805::writeMultipleRegisters(uint8_t addr, uint8_t * values, uint8_t len)
{
	...
	{
		I2C_0_write1ByteRegister(RV1805_ADDR, addr, values[i]);
	}
	return(true);
}
...
{
		I2C_0_readDataBlock(RV1805_ADDR, addr, dest, len);
